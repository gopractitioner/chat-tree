...
export const CustomNode = ({ data }: { data: any }) => {
  ...
  const timestampMs = (() => {
    ...
    // Back-compat: treat small values as seconds
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className={`w-2 h-2 rounded-full mr-2 ${data.role === 'user' ? 'bg-yellow-400' : data.role === 'assistant' ? 'bg-gray-400' : 'bg-gray-400'
              }`} />
        ...
        {/* Message content with markdown support - scrollable when long (all providers) */}
        <div className={`mt-2 text-sm text-gray-700 ${isExpanded
            ? 'h-[calc(100%-100px)] overflow-y-auto nowheel'
            : 'min-h-0 max-h-[72px] overflow-y-auto'
          }`}
